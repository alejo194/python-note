...
from app.web_framework import get, post

from app.models import User, Comment, Blog, next_id

from app.apis import Page, APIValueError, APIResourceNotFoundError, APIError, APIPermissionError
...
async def index(*, page='1'):
    ...
    if (not num) and num == 0:
        ...
        #否则，根据计算出来的offset(取的初始条目index)和limit(取的条目)，来取出条目
        blogs = await Blog.findAll(orderBy='created_at_desc', limit=(page.offset, page.limit))
    # 返回给浏览器
    return {
...
async def api_blogs(*, page='1'):
    # 获取博客信息，调用位置：manage_blog_edit.html 40行
    '''
...
async def get_blog(id):
    ...
    # markdown2是个扩展模块，这里把博客正文和评论讨人到markdonw2中
    for c in comments:
        c.html_content = text2html(c.html_content)
...
async def api_delete_blog(id, request):
    # 删除一条博客
    logger.info("删除博客的ID为： %s" %id)
    # 先检查是否是管理员操作，只有管理员才能删除评论权限
    check_admin(request)
    # 查询一下评论id是否有对应的评论
